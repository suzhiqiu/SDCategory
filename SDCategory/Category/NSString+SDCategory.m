...
//字典或数组转JSON字符串
+ (NSString*)sd_dictionaryToJsonString:(id)dic
{
    if(!dic)
    {
        return @"";
    ...
    NSString *jsonStr=[[NSString alloc]initWithData:jsonData encoding:NSUTF8StringEncoding];//去除空格和回车：
...
//查找子串开始位置
- (NSUInteger)sd_findSubStrIndex:(NSString *)subStr
{
...
//获取url value  findKey:
-(NSString *)sd_getURLValueByKey:(NSString *)findKey
{
    if (![findKey hasSuffix:@"="]) {
        findKey = [NSString stringWithFormat:@"%@=", findKey];
    }
    NSString *value=@"";
    if ([self sd_contains:@"?"]) {
        NSUInteger location = [self sd_findSubStrIndex:@"?"];
        NSString *tempStr = [self stringByReplacingCharactersInRange: NSMakeRange(0, location+1) withString:@""];
        NSArray *paramsArray = [tempStr componentsSeparatedByString:@"&"];
        for (NSString *key in paramsArray)
        {
            if( [key sd_contains:findKey])
            {
                if ([[key substringToIndex:findKey.length] sd_equals:findKey])
                {
                    value=[key substringFromIndex:findKey.length];
                    break;
                }
            }
        }
...
//url 尾部添加 key value
+(NSString *)sd_addKeyValueByUrlTail:(NSString *)urlStr  key:(NSString *)key  value:(NSString *)value
{
    NSString *newStrl=@"";
    
    if ([urlStr sd_contains:@"?"])
    {
        newStrl = [urlStr stringByAppendingFormat:@"&%@=%@",key,value];
    }else {
        newStrl = [urlStr stringByAppendingFormat:@"?%@=%@",key,value];
    }
    return newStrl;
